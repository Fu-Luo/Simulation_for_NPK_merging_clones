...
timepoint=0;
clonesizes=0;
maxclonesize=200 %mean 243 median 137 largest clones in each slide. 12 weeks np 
...
if mergingevent(1,1)>0
            %check all clones
            %check large clones size=20 (top 5% clones in np 12 weeks)
       outputmatrix(testcount,1)=size(mergingevent,1)/sum(clonesizes>20);
       outputmatrix(testcount,2)=size(mergingevent,1);
       outputmatrix(testcount,3)=sum(clonesizes>20);
...
writematrix(outputmatrix,"NP3to12largeclone20.csv")